...
class BM25System(SearchSystem):
    ...
    def search(self, queries: List[str], top_k: int = 10) -> List[RankedResults]:
        """
        ...
        Returns:
            A list of ranked results, one list per query.
        """
        index_dir = os.path.join(ARTIFACTS_DIR, self.name.lower(), "index")
        ...
        all_results: List[RankedResults] = []
    ...
    def save_run(self, results: List[RankedResults], output_filename: str) -> None:
        """
        Save ranked retrieval results in plain tab-separated format.

        Args:
            results: Ranked results per query.
        ...
        print(f"[{self.name}] Saving results to {output_path}...")
        with open(output_path, "w", encoding="utf-8") as output_file:
            for query_idx, ranked_docs in enumerate(results, start=1):
                for rank, (doc_id, score) in enumerate(ranked_docs, start=1):
                    # Columns: query_id, doc_id, rank, score
                    output_file.write(f"{query_idx}\t{doc_id}\t{rank}\t{score:.6f}\n")